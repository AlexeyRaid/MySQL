...
from analyticdb.et_sales as s
         ...
         left join analyticdb.zpn_sched_conditions_final as graf on s.period between graf.DTStart and graf.DTEnd and
                                                                    s.employee = graf.fio_ref
         left join analyticdb.zpn_paycond_zavotd as pay on s.period between pay.date_from and pay.date_to
    and s.employee = pay.ref_donor
...
where s.period >= '2023-02-01 00:00'
  and pay.share_donor_assigned_services is not null
  and (s.price <> 0
