...
from analyticdb.et_sales as s
         RIGHT JOIN gs_employee AS em ON em.ref_key = s.executor
         LEFT JOIN zpn_schedule AS sh ON em.fio = sh.fio AND s.period >= sh.DTStart AND s.period < sh.DTEnd
         left join analyticdb.gs_posts as post on
        (SELECT l.priority
         FROM zpn_levelsemployees AS l
         WHERE em.fio = l.fio
