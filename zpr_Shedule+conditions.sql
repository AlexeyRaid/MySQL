select gr.employee,
       emp.fio,
       gr.day, gr.time_start, gr.time_end, gr.`year_month`, gr.shift, lev.level as level, gr.post,
...
left join analyticdb.zpr_level as lev on emp.fio=lev.fio
                                     ...
                                     and DATE_FORMAT(concat(date(concat(DATE_FORMAT(gr.`year_month`, '%Y-%m-'),gr.day)), " ", gr.time_start),'%Y-%m-%d %H:%i') >= lev.date_from and DATE_FORMAT(concat(date(concat(DATE_FORMAT(gr.`year_month`, '%Y-%m-'),gr.day)), " ", gr.time_start),'%Y-%m-%d %H:%i') <= lev.date_to
